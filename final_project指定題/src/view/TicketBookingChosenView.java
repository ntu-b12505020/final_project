...
/**
 * TicketBookingChosenView 類別：訂票選擇畫面與邏輯
 */
public class TicketBookingChosenView {
    private VBox view; // 主畫面 VBox
    private List<Seat> selectedSeats = new ArrayList<>(); // 已選座位
    private int money = 0; // 總金額

    /**
     * 建構子（多載）：僅提供 userEmail 時
     * 
     * @param stage     主視窗
     * @param userEmail 使用者信箱
     */
    public TicketBookingChosenView(Stage stage, String userEmail) {
        this(stage, userEmail, null, null);
    }
     ...
     * @param stage            主視窗
     * @param userEmail        使用者信箱
     * @param selectedMovie    選擇的電影
     * @param selectedShowtime 選擇的場次
     */
    public TicketBookingChosenView(Stage stage, String userEmail, Movie selectedMovie, Showtime selectedShowtime) {
        Label movieLabel = new Label("Movie: " + selectedMovie);

        Label showtimeLabel = new Label("Showtime: " + selectedShowtime);
        ...
        plusrButton.setOnAction(e -> {
            if (Integer.parseInt(numberrLabel.getText()) + Integer.parseInt(numbercLabel.getText()) < Database
                    .getSeatNumByShowtime(selectedShowtime)) {
                String newnum = Integer.toString(Integer.parseInt(numberrLabel.getText()) + 1);
                numberrLabel.setText(newnum);
                money = money + 320;
            }
        ...
        pluspBtn.setOnAction(e -> {
            if (Integer.parseInt(numberrLabel.getText()) + Integer.parseInt(numbercLabel.getText()) != 0) {
        ...
        bookButton.setOnAction(e -> {
            ...
            if (selectedSeats.size() != Integer.parseInt(numberrLabel.getText())
                    + Integer.parseInt(numbercLabel.getText())) {
                showWarning("Please select seat!");
                return;
            }
        ...
        view = new VBox(10, movieLabel, showtimeLabel, bookHBox, seatLabel, scrollPane, bookBox);
